...
# 나뭇잎 점프대 (2026-06-05 프로토타입) — 설치형 발동 표지판(SkillSign)으로 끈끈이 직전 빈 바닥에
# 설치한다. 그 셀에 처음 도착한 적격 개미가 전방 LEAF_JUMP_CELLS타일을 포물선으로 비행(LeafJumpState,
# 낙하 모션)해 끈끈이 너머 바닥에 착지한다. 비행 내내 끈끈이 면역(Ant.is_jump_immune). 재사용형
# (SkillSign.REUSABLE_SIGNS) — 도착하는 모든 적격 개미가 반복 점프(무리 통행).
#
# 부여(인벤토리 차감)는 설치 시점(SkillToolbar._place_sign), 발동은 SkillSign._physics_process가
# can_apply 재검사 후 skill.apply 직접 호출. 무장(basher/cutter) 같은 지연 분기 없음 — 즉시 발사.
...
func can_apply(ant: Ant) -> bool:
	...
	if not (s is WalkerState or s is CarryingState):
		return false
	# 착지 가능한 전방 바닥이 있어야 발동 — 허공/벽으로 점프 금지. SkillSign 발동 시점에 재검사된다.
