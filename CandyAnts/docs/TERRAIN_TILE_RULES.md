...
지형은 **under-surface(최상단 전환 띠) / interior(깊은 내부)** 두 tier로 구성된다. 다른 stage·theme로 확장할 때
따라야 할 규약을 정의한다. 신규 타일 자산 발주, `StageLayoutBuilder` 수정, 새 stage 레이아웃 작성은 반드시 이 문서를 우선한다.

> **개정 이력 — surface tier 제거 (terrain-tier-restructure, 2026-06-01)**
...
**대상**: ground terrain의 under-surface / interior 2-tier 타일링과 그 painterly cookie 아트 디렉션 (현재 `cookie_crust` theme 1종).

**대상에 포함** (별도 구조라 별도 섹션):
- **모래 쌓기(Sand-Mound) 동적 스킬 타일** — §11. ground terrain과 정반대(세로 스택·1칸 폭·정사각형)라 본문 §1~§10의 가로 아틀라스 규약을 적용하지 않는다. **sand-mound는 자체 surface/under/background 3-tier를 그대로 유지한다**(ground terrain의 surface 제거와 독립 — §9 참조).
...
- 회화적이고 부드러운 캔디-쿠키 톤.
- 걷는 면(지형 최상단)은 under-surface 전환 띠 — 쿠키 측면 크러스트가 상단 가장자리에 보이고 아래로 초콜릿 내부로 흐른다.
...
**시각 tier 2개** (셀이 어떻게 그려지는지)

| Tier | 텍스처 | 역할 |
| --- | --- | --- |
| under-surface | `cookie_tile_under_surface.png` | **지형 최상단**(공기와 맞닿는 노출 면). 상단은 쿠키 측면 가장자리, 하단은 초콜릿 내부로 자연스럽게 이어진다. |
| interior | `cookie_tile_background.png` | 깊은 초콜릿 본체. 쿠키 상단 가장자리를 포함하지 않으며 수직 반복 시 가로 단턱이 보이지 않는다. |

> 톱다운 `surface` tier는 제거됐다. 더 이상 별도의 톱다운 쿠키 바닥 텍스처/타일/오버레이가 없다.

**레벨 데이터에 직접 쓰는 tile type은 2종**:

| `tile_map` 값 | 충돌 | 시각 동작 |
| --- | --- | --- |
| `solid` | 있음 | `_add_solid_visual()`로 그려짐. 베이스 텍스처는 §3 자동 추론으로 under-surface 또는 interior 결정. |
| `background` | 없음 | interior 텍스처를 시각 전용으로 채움. 빈 공간이 검게 비치는 걸 막는다. |

**under-surface / interior는 별도 tile type이 아니다** — `solid` 셀이 위치(노출 여부)에 따라 builder가 자동으로 선택한다.
...
- `solid` 셀 바로 위 칸이 **레이아웃에 존재하지 않으면**(`not tile_map.has(above_key)` = 진짜 빈 칸/공기) → 노출 최상단 → `cookie_tile_under_surface.png`.
- 위 칸이 레이아웃에 **존재하면**(solid / slope_* / plant / background 무엇이든 = 가려진 셀) → `cookie_tile_background.png`(interior).
...
노출 최상단의 룩은 베이스 텍스처(under-surface) 자체가 담당한다. **`SurfaceSprite` 노드는 더 이상 생성되지 않는다.**
...
**Atlas 가로 규격**: `336 × 48` (= 48² 정사각 셀 7개 가로 variant). `_configure_repeating_region()`이 `posmod(cell.x, columns)`로 cell.x에 따라 variant를 선택한다 — 같은 row를 따라가도 가로 반복이 단조롭게 보이지 않게 한다.

**SoT 도구**: `scripts/tools/process_new_tiles.py`.
- 원본 painting을 `make_seamless()`로 가로 이음새를 자동 블렌딩한 뒤 `336 × 48` atlas로 출력.
- under-surface / interior tier 텍스처를 생성한다. (이전 surface tier 텍스처는 ground에서 미사용 — 슬로프 대각면이 `cookie_tile_surface.png`를 계속 쓰므로 파일 자체는 보존.)
- 같은 출력을 `cookie_tile_<tier>.png` (live)와 `cookie_tile_<tier>_painted.png` (백업) 양쪽에 저장. flip 변종도 동시 생성.

**`_painted` 백업 컨벤션**: live 파일은 추후 다른 후처리가 들어갈 수 있다. `_painted` 변종은 painterly 원본을 항상 보존해두는 컨벤션이다. 도구가 자동으로 동기화하므로 수작업으로 한쪽만 갱신하지 않는다.
...
1. **Under-Surface 하단 ↔ Interior 상단** — under-surface의 초콜릿 전환부가 interior 상단으로 끊김 없이 연결된다.
...
- tier 텍스처가 서로 **다른 원본 painting**에서 독립 crop된다. 세로 정합은 원본 painting들의 톤 일관성에 의존한다.
- 따라서 **새 자산을 발주할 때 §5의 U→I→I 체인을 요구사항으로 명시**해야 한다. 도구가 통과시킨다고 시각 정합이 보장되는 게 아니다.
...
4. 시각 추론(노출 최상단=under-surface, 그 외=interior)은 builder가 자동 — 추가 작업 없음.

**새 theme 추가**
1. 같은 2-tier 모델(under-surface / interior)을 유지한 painting을 의뢰한다. §1 painterly 디렉션과 §5 세로 체인 조건을 발주서에 박제한다.
2. `process_new_tiles.py`에 새 theme 분기를 추가하거나, 그에 상응하는 atlas 2장을 같은 규격(`336 × 48`)으로 직접 생성한다.
...
- §5 세로 체인 U→I→I 정합 요구.
- 가능하면 under-surface / interior 원본을 **단일 painting의 2개 band**로 통합 요청 — 세로 정합이 원본 시점에서 보장된다.
...
| 테스트 | 검증 항목 |
| --- | --- |
| `_test_texture_tiers_and_visual_only()` | `solid`만 충돌 등록 / `background`는 visual-only / **노출 최상단 solid = under-surface** / **가려진 solid = interior** / **`SurfaceSprite` 오버레이 부재** / exposure edge case(stacked / under-slope / under-background / isolated-exposed) |
| `_check_stage()` × stage01/02/03 | 실제 레이아웃 마이그레이션 불변 — **`surface` 타일 타입 부재** / 점유 셀 == collision tile 셀 / under-surface·interior tier 정합 / `SurfaceSprite` 부재 |
...
1. **세로 연속성 (§5)** — `process_new_tiles.py`에 vertical blend 단계가 없다. 개선안: 단일 source painting의 band 통합 또는 도구에 `make_seamless_vertical` 추가.
...
- Under-Surface → Interior 전환에 띠 단절이 보임.
- 파스텔 배경과 충돌하는 고대비 검정 또는 네온 톤이 사용됨.
- **레이아웃 `.tres`에 `"surface"` 타일 값이 존재함** (폐기된 타입 — solid/background만 허용).
- **`solid` 셀 위에 `SurfaceSprite` 오버레이가 생성됨** (제거된 레거시 — 노출 최상단은 under-surface 베이스 텍스처로 표현).
- **노출 최상단 solid가 interior 텍스처로 그려짐**(또는 가려진 solid가 under-surface로 그려짐) — exposure 술어(`not map.has(above_key)`) 위반.
