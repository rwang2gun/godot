...
- **파라미터 가이드(§2.2, 실측 조정)**: Ch1 hp4~5 / ants는 hp보다 여유 / time 90~110s.
  - **개정 (사용자 결정 2026-06-10, 설계 §2.2 Ch1 고정 지침으로 대체)**: candy_hp=**5 고정** + total_ants=**5+영구 소비 설치물 수(여분 0)** + 소비형 인벤토리=필요 수 그대로. 적용: id11 5/5(climber 5) · id12 6/5(blocker 1) · id13 6/5(blocker 1+climber 5) · id14 6/5(floater 1, 2→1 축소). stage01·02 정합화는 B2 sweep.
