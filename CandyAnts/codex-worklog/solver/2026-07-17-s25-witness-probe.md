...
## 손-probe 배달 규명 (2026-07-18, 커밋 후속·미커밋 probe) — **배달 직전까지, escape 충돌만 남음**

> 사용자 힌트 2건(전부 사용자 실해 미참조, probe 실측): ① "바닥 타일 중 **위에 천정(solid) 있는 곳**에 사다리"
> ② "사다리 충돌=항상 상승이라 blocker 불요, 사다리는 타일 푯말이라 미리 배치 가능".

- **핵심 발견 = col15 2-사다리 직통 배달**: 운반개미가 닿는 col15 위 5칸에 **박스 바닥(15,5)이 천정**. 즉
  `sand_mound(15,13)`(분지 row14→cap (15,11) row11 플랫폼) + `sand_mound(15,10)`(row11→**cap (15,5) 박스바닥
  관통**→(15,4) 박스 내부). 사다리 2개로 배달(escape 1 포함 sand_mound 3 = 사용자 개수 일치). **실측: 운반개미
  ant6가 candy 든 채 박스 내부 (15,4) 도달** — 배달 메커니즘 전구간 관통 실증. (내 deliver-below routing은 "천정
  ≤3행 위"만 봐서 이 5행-위 박스바닥을 놓침 → routing 확장 여지.)
- **유일 잔여 벽 = escape 사다리 vs 배달 경로 충돌**: (15,4) 진입 운반개미가 home(19,4)으로 우향 보행 중
  **스폰-탈출 사다리(cols15~19 중 어딘가)를 밟고 다시 climb-out**. 기하: 진입 col15→home col19 경로(col16~18)와
  탈출 사다리 위치가 겹침. escape@(19,4)=home셀 두면 스폰 탈출 자체 깨짐(picked=0), @15/16이면 배달 경로 겹침.
- **다음 세션 착수점**: ① 배달 진입을 **col19(home) 직통**(운반개미를 row11 플랫폼서 col19까지 — 단 사다리
  emergence 후 col15서 반전하는 문제 규명 필요; fall-착지 개미는 col20까지 자유보행하나 ladder-emergence는 반전)
  ② 또는 탈출을 **slideL/R(대각 계단)**로 바꿔 사다리-충돌 경로 분리 ③ deliver-below를 "천정 ≤5행 위(박스바닥
  직통)"까지 확장. push 완료(7fe6c5e origin/main). RL 격리 유지.
