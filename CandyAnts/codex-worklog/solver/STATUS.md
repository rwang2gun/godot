...
## 2차 스윕 진행 스냅샷 (2026-07-12 저녁, 러너 계속 실행 중)

> 시도별 분리 기록 도입(`139f593`: stageNN.attemptK.log + attempts.jsonl 이력 SoT) 후
> `--stages 3-25` 재개. **S3~S15 완료, S16 진행 중**(러너 PID 분리 실행 — 세션 종료와 무관하게
> 계속 돌며 sweep_out/에 durable 기록, 중단돼도 재실행 시 지문-결속 스킵으로 이어짐).
- **PASS(≥2/3)**: S3(2/3)·S4·S5·S7·S8·S11·S12·S13(각 3/3) — 8개
- **부분(1/3, 해 등재)**: S6(59.8분)·**S9(45.4분, 미등재였던 스테이지 첫 해 — bridge 구조를
  순수 RL이 발견)** — 2개
- **무클리어(0/3, partial 기록)**: S10(17분)·S14(51분)·S15(36.5분) — Ch1 후반 다단 구조가 벽
- **레지스트리 15→24해/12→14스테이지**: 신규 배치 발견 S5(+2)·S7(+2)·S8(+2)·S11(+1) +
