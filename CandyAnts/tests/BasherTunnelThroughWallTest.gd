...
# Phase 18 — Basher 4-cell wall 통과 + candy 회수.
# 시나리오: home(5,21) → walker → wall(12,21..15,21) 직면 → basher 적용 →
#          4 cell wall 제거 → walker 진행 → candy(25,21) → carrying → home → saved+1.
#
# PASS (30s 내):
#  (1) saved_pieces >= 1
#  (2) wall cell 4개 (12,21)..(15,21) 모두 get_cell_kind == "" (제거 확인)
#  (3) has_tile(cell) == false for each wall cell

const DEADLINE_FRAMES: int = 1800
const TRIGGER_X: float = 11.0 * 32.0   # cell 11 left edge — wall 직전
...
func _ready() -> void:
	print("[BasherTunnelThroughWallTest] driver ready")
...
func _apply_when_ready() -> void:
	...
	skill.apply(_ant)
	_applied = true
	print("[BasherTunnelThroughWallTest] applied basher at frame=%d pos=%s" % [_frame, _ant.global_position])
...
func _poll_pass() -> void:
	...
	if saved < 1:
		return
	# (1) saved >= 1 만족. (2)(3) wall cell 4개 검증.
	for x in range(12, 16):
		var cell: Vector2i = Vector2i(x, 21)
		var kind: String = _terrain.get_cell_kind(cell)
		if kind != "":
			_fail("wall cell %s 잔존 (kind=%s)" % [str(cell), kind])
			return
		if _terrain.has_tile(cell):
			_fail("wall cell %s has_tile=true" % str(cell))
			return
	print("[BasherTunnelThroughWallTest] PASS saved=%d frame=%d" % [saved, _frame])
...
func _fail(msg: String) -> void:
	push_error("[BasherTunnelThroughWallTest] FAIL: " + msg)
	_result_emitted = true
	get_tree().quit(1)
