...
# 발동 표지판 프로토타입 검증 — basher (2026-06-05).
# 개미를 직접 탭하지 않고 SkillToolbar._place_sign로 "흙벽 앞 셀"(11,21)에 basher 표지판 설치 →
# 도착 개미가 BasherSkill.apply(전방 벽 직면이면 즉시 굴착, 열림이면 armed 후 벽 도달 시 자동 굴착) →
# 흙벽 4칸(12,21)..(15,21) 제거 + 표지판 소비(queue_free) + 사탕 회수.
# PASS: 표지판 설치 성공 AND 벽 4칸 모두 비점유 AND 잔존 SkillSign 0.

const DEADLINE_FRAMES: int = 2400
const SIGN_X: float = 11.0 * 32.0 + 16.0   # cell 11 중앙 — 흙벽(cell 12) 직전
...
func _ready() -> void:
	...
	print("[BasherSignTest] driver ready")
...
func _place_when_ready() -> void:
	...
	if _ant.global_position.x >= SIGN_X:
		return
	var ok: bool = _toolbar._place_sign("basher", Vector2(SIGN_X, _ant.global_position.y))
	if not ok:
		_fail("_place_sign returned false (cell occupied / no terrain)")
		return
	_placed = true
	print("[BasherSignTest] sign placed at frame=%d ant_x=%.1f signs=%d" % [_frame, _ant.global_position.x, _sign_count()])

func _poll_bashed() -> void:
	if not _placed:
		return
	if _walls_open() == WALL_CELLS.size() and _sign_count() == 0:
		print("[BasherSignTest] PASS walls removed + sign consumed frame=%d" % _frame)
		_done = true
		get_tree().quit(0)
...
func _on_failed(result: Dictionary) -> void:
	if _done:
		return
	if _walls_open() == WALL_CELLS.size() and _sign_count() == 0:
		print("[BasherSignTest] PASS (post-failed) reason=%s" % result["reason"])
		_done = true
		get_tree().quit(0)
		return
...
func _fail(msg: String) -> void:
	print("[BasherSignTest] FAIL %s" % msg)
	_done = true
	get_tree().quit(1)
