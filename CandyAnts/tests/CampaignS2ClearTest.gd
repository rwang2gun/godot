...
# Campaign S2 "오르막" — distributor 분배 방식 클리어 가능성 검증 (레벨 재설계 rev2, 2026-06-02 세션 3).
# 플레이어 모사:
#   1) 최전방(frontmost) 개미 1마리에 floater + distributor 부여 → col11 SettlementMarker에 정착 →
#      이후 지나가는 모든 개미에게 floater 전이.
...
#   - floater를 개별 부여하지 않았는데 saved==5면 → 분배자 floater 전이 경로가 실제로 동작함을 입증.
...
func _drive() -> void:
	# 1) 분배자 미선정 → 최전방 walker를 floater+distributor 분배자로. 마커(col11) 도달 전 선정 보장.
	if _distributor_id == 0:
		...
		if front != null:
			...
			var ds: DistributorSkill = DistributorSkill.new()
			if fl.can_apply(front) and ds.can_apply(front):
				fl.apply(front)
				ds.apply(front)
				_distributor_id = front.get_instance_id()
				print("[CampaignS2ClearTest] distributor(+floater) → %s pos=%s frame=%d" % [
