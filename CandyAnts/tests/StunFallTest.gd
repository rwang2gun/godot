...
# 기절(stun) 낙하 검증 (2026-06-02) — 5칸+ 자유낙하 + floater 미보유 → DeadState(기절, ~1초 후 queue_free) +
# 운반 중이면 candy_piece_lost 정산. 5칸 미만 또는 floater 보유는 기절 없이 보행/운반 복귀(생존).
# bare 씬(StageLayoutBuilder ancestor 없음) → Ant._cell_size=48 기본 → 임계 240px.
#   A: 비-floater 운반, drop≈360(>240) → 기절(DeadState 관측) + 사탕 lost + 결국 제거.
...
func _physics_process(_delta: float) -> void:
	_frame += 1
	# DeadState는 ~1초 후 queue_free라 freed 전에 매 frame 폴링으로 포착.
	if _is_dead(_a):
		_saw_dead_a = true
	if _is_dead(_b):
		_saw_dead_b = true
...
func _evaluate() -> void:
	...
	# A — 기절: DeadState 관측 + 사탕 lost + 결국 queue_free(제거).
	if not _saw_dead_a:
	...
	if is_instance_valid(_a):
		fails.append("A: ~1초 후에도 제거 안 됨(queue_free 누락)")
	...
	if fails.size() > 0:
		...
		print("[StunFallTest] PASS — A 기절+lost+제거 / B 저낙하 생존 / C floater 생존(사탕 유지)")
		get_tree().quit(0)
