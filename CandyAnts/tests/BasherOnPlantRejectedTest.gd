...
# Phase 19 — 메카닉 분리 검증 역방향 (Basher→plant cross-kind block).
...
# Driver: ant body_cell (13,21) (x=13*32+16=432, y=699), direction=+1.
# basher 적용 → forward (14,21) plant → _basher_forward_has_earth=false → _aborted → Walker 복귀.
#
# PASS (30s 내):
#  (1) basher 직후 첫 2 tick(~0.36s) 후 ant state == WalkerState
#  (2) plant wall cell 3개 (14,21)(15,21)(16,21) kind="plant" 유지
#  (3) static count 무변동
...
const PLANT_WALL_CELLS: Array[Vector2i] = [
	Vector2i(14, 21), Vector2i(15, 21), Vector2i(16, 21)
]
...
func _ready() -> void:
	...
	_ant.global_position = Vector2(13.0 * 32.0 + 16.0, 699.0)
	_ant.direction = 1
	print("[BasherOnPlantRejectedTest] driver ready pre_static=%d" % _pre_static_count)
...
func _apply_when_ready() -> void:
	...
	print("[BasherOnPlantRejectedTest] applied basher frame=%d pos=%s" % [_frame, _ant.global_position])
...
func _poll_pass() -> void:
	...
	if not (s is WalkerState):
		...
		return
	for c in PLANT_WALL_CELLS:
		var k: String = _terrain.get_cell_kind(c)
		if k != "plant":
			_fail("plant wall %s kind 변동 — kind=%s (cross-kind destruction 검출)" % [str(c), k])
			return
	var post: int = _terrain_static_count()
	if post != _pre_static_count:
		_fail("static body 카운트 변동 — pre=%d post=%d" % [_pre_static_count, post])
		return
	print("[BasherOnPlantRejectedTest] PASS frame=%d ant_pos=%s static=%d" % [_frame, _ant.global_position, post])
...
func _fail(msg: String) -> void:
	push_error("[BasherOnPlantRejectedTest] FAIL: " + msg)
	_result_emitted = true
	get_tree().quit(1)
