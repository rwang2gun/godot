...
- **codex 호출 모델 = 항상 사용 가능한 최신** (2026-07-11 지침): 모델 핀 SoT는 `~/.codex/config.toml`의 `model`. 세션에서 codex(리뷰/task)를 처음 호출하기 전에 **1회 최신 확인** — 현행 핀보다 높은 마이너 버전 1~2개(예: 핀이 `gpt-5.5`면 `gpt-5.6`·`gpt-5.6-codex`)를 `codex-companion.mjs task --model <후보> --effort minimal "ping"`으로 시도. 성공하면 config 핀을 그 모델로 올리고 사용자에게 1줄 보고, 400(`not supported when using Codex with a ChatGPT account`)이면 현행 핀 유지. ChatGPT-계정 Codex는 모델 목록 조회 API가 없어 probe 방식이 유일함. `adversarial-review`도 `--model`(단축 `-m`)을 받으나(help 미문서), 핀을 올리는 쪽이 세션 전체에 일관 적용되므로 우선
